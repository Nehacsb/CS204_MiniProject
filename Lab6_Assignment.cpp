...
void one_bit_dynamic()
{
    ...
    for (int i = 0; i < no_of_branches; i++)
    {
        if (branch_prediction[branch_prediction.size() - 1].compare(branch_outcome[i]) == 0)
        {
        ...
        {
            branch_prediction.push_back(branch_outcome[i]);
            miss++;
        }
    }
    accuracy = (hit * 100 / n);
...
void two_bit_dynamic()
{
    ...
    {
        string dlep;
        string hlep = branch_prediction[branch_prediction.size() - 2] + branch_prediction[branch_prediction.size() - 1];
        if (hlep == "NN" || hlep == "NT")
            dlep = "N";
        else
            dlep = "T";
        if (dlep.compare(branch_outcome[i]) == 0)
        {
            if (branch_prediction[branch_prediction.size() - 1] != branch_outcome[i])
            {
                branch_prediction[branch_prediction.size() - 1] = branch_outcome[i];
            }
